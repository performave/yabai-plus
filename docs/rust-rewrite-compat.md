...
| `query --windows` fields | Supported (daemon augments the pure serializer with live AX/SkyLight reads before serving): the core fields plus `opacity`, `role`, `subrole`, `level`, `layer`, `can-move`, `can-resize`. Still deferred: `sub-level`/`sub-layer` (C reads sub-level via a fragile magic-id `mach_msg`), and `root-window`/`has-ax-reference`/`is-native-fullscreen`/`is-minimized`/`is-hidden`/`is-grabbed` (mostly always-false/true for tiled windows since those states remove a window from its tree — meaningful only once off-tree windows are listed). Requesting a deferred field errors "not available from pure state" | Deferred fields need the fragile `mach_msg` sub-level read or off-tree window listing |
| `query --windows` scope | Tiled windows plus off-tree floating/sticky/scratchpad windows are listed (the daemon AX-reads the latter's frames). Minimized and native-fullscreen windows are still omitted (they leave the tree and aren't in the floating/sticky/scratchpad sets — would need the daemon to track their frames too) | Minor: minimized/native-fullscreen windows not yet listed |
| `query --spaces` / `--displays` fields | `query --spaces` reports `id`, `index` (mission-control index), `label`, `type`, `display`, `windows`, `first-window`, `last-window`, `has-focus`, `is-visible`, `is-native-fullscreen` (SLSSpaceGetType). `query --displays` reports `id`, `uuid` (CGDisplay UUID), `index`, `label`, `frame`, `spaces`, `has-focus`. Deferred: space `uuid` (plumbing exists but gated — `SLSSpaceCopyName` returns empty on macOS 15 in this setup) | Minor: space `uuid` gated pending an SLSSpaceCopyName fix |
