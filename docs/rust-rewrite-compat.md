...
| `query --windows` fields | Supported (daemon augments the pure serializer with live AX/SkyLight reads before serving): the core fields plus `opacity`, `role`, `subrole`, `can-move`, `can-resize`. Still deferred: `level`, `sub-level`, `layer`, `sub-layer` (fragile version-specific private SkyLight reads — `level` uses `SLSWindowQuery*` on Sequoia, `sub-level` a raw magic-id `mach_msg`), and `root-window`, `has-ax-reference`, `is-native-fullscreen`, `is-minimized`, `is-hidden`, `is-grabbed` (mostly always-false/true for tiled windows since those states remove a window from its tree). Requesting a deferred field errors "not available from pure state" | Deferred fields need the fragile private SkyLight reads or off-tree window listing |
