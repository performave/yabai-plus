...
| `query --spaces` / `--displays` fields | `query --spaces` reports `id`, `index` (mission-control index), `label`, `type`, `display`, `windows`, `first-window`, `last-window`, `has-focus`, `is-visible`. `query --displays` reports `id`, `index`, `label`, `frame`, `spaces`, `has-focus`. Deferred: space/display `uuid` (needs a SkyLight/CG UUID read pushed by the daemon) and space `is-native-fullscreen` | Minor: `uuid` and space `is-native-fullscreen` need a daemon-pushed live read |
