...
*yabai* [*--load-sa*|*--uninstall-sa*|*--check-sa*|*--install-sudoers*|*--uninstall-sudoers*|*--install-service*|*--uninstall-service*|*--start-service*|*--restart-service*|*--stop-service*|*--message*,*-m* 'msg'|*--config*,*-c* 'config_file'|*--verbose*,*-V*|*--version*,*-v*|*--help*,*-h*]
...
*--check-sa*::
    Report whether the scripting-addition is loaded and healthy. +
    Talks to the payload running inside Dock.app directly, so it reflects the
    real running state rather than whether files are installed. +
    Does not require root and does not re-inject. +
    Exits 0 only when the payload responds, matches this build's version, and
    advertises full support for the current macOS; non-zero otherwise.

*--install-sudoers*::
    Writes a sha256-pinned passwordless sudoers rule that lets the running user
    invoke *sudo yabai --load-sa* without a password. Must be run as root
    (typically *sudo yabai --install-sudoers*). +
    The rule is pinned to this exact binary's sha256 and only authorizes the
    *--load-sa* subcommand, so it stops authorizing the moment the binary
    changes. It lets the launchd service -- which has no tty -- inject the
    scripting-addition unattended; without it, window moves fall back to the
    blocking accessibility path. +
    Path is /private/etc/sudoers.d/yabai. The generated rule is validated with
    *visudo -c* before being moved into place, so a malformed rule can never
    lock sudo.

*--uninstall-sudoers*::
    Removes the passwordless *--load-sa* sudoers rule. Must be run as root
    (typically *sudo yabai --uninstall-sudoers*). +
    Path is /private/etc/sudoers.d/yabai.
...
*manage* ['<BOOL_SEL>']::
    Automatically manage (tile) eligible windows. +
    'on' (default): Eligible windows are tiled, matching upstream behaviour. +
    'off': Hybrid mode. Windows are only tiled when matched by a rule that sets
    'manage=on'; everything else floats, including pre-existing windows at startup. +
    Toggling this at runtime re-evaluates existing windows, so a restart is not required.

*window_sublayer_auto* ['<BOOL_SEL>']::
    Control how the automatic ('auto') window sub-layer is resolved. +
    'on' (default): Managed windows render below floating windows, matching upstream
    behaviour (floats sit on top of tiles). +
    'off': All windows using the 'auto' layer resolve to the normal sub-layer, so
    floating windows no longer render above tiled windows. +
    Toggling this at runtime re-applies the layer to existing windows.
